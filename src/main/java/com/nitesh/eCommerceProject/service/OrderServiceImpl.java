...
import com.nitesh.eCommerceProject.entity.OrderStatusValuesEnum;
...
import com.nitesh.eCommerceProject.repository.OrderDao;
...
public class OrderServiceImpl implements OrderService {
		...
		if(loggedInCustomer != null) {
			...
			if(productsInCart.size()!=0) {
				if((usersCardNumber.equals(userGivenCardNumber)) 
					...
					newOrder.setOrderStatus(OrderStatusValuesEnum.SUCCESS);
					System.out.println(usersCardNumber);
					...
					for(CartItem cartItem : cartItemsList ) {
						...
						if(remainingQuantity < 0 || cartItem.getCartProduct().getStatus() == ProductStatusEnum.OUTOFSTOCK) {
						...
						if(cartItem.getCartProduct().getQuantity()==0) {
							cartItem.getCartProduct().setStatus(ProductStatusEnum.OUTOFSTOCK);
						}
					}
					...
					newOrder.setAddress(loggedInCustomer.getAddress().get(odto.getAddressType()));
					newOrder.setDate(LocalDate.now());
					newOrder.setOrderStatus(OrderStatusValuesEnum.PENDING);
					cartservicei.clearCart(token);
					return oDao.save(newOrder);
	...
	public Order cancelOrderByOrderId(Integer OrderId,String token) throws OrderException {
		...
		if(order.getCustomer().getCustomerId()==cs.getLoggedInCustomerDetails(token).getCustomerId()) {
			if(order.getOrderStatus()==OrderStatusValuesEnum.PENDING) {
				order.setOrderStatus(OrderStatusValuesEnum.CANCELLED);
				oDao.save(order);
				return order;
			}
			else if(order.getOrderStatus()==OrderStatusValuesEnum.SUCCESS) {
				order.setOrderStatus(OrderStatusValuesEnum.CANCELLED);
				...
				for(CartItem cartItem : cartItemsList ) {
					...
					if(cartItem.getCartProduct().getStatus() == ProductStatusEnum.OUTOFSTOCK) {
						cartItem.getCartProduct().setStatus(ProductStatusEnum.AVAILABLE);
					}
				}
...
//			System.out.println(loggedInCustomer);
			if((usersCardNumber.equals(userGivenCardNumber)) 
				...
				existingOrder.setCardNumber(orderdto.getCardNumber().getCardNumber());
				existingOrder.setAddress(existingOrder.getCustomer().getAddress().get(orderdto.getAddressType()));
				existingOrder.setOrderStatus(OrderStatusValuesEnum.SUCCESS);
